...
func NewAuthService(repo ports.IUserRepository, hasher IHasher, tokenRepo ports.TokenRepository, jwtKey []byte, logger *slog.Logger) *AuthService {
	return &AuthService{userRepo: repo, hasher: hasher, tokenRepo: tokenRepo, jwtKey: jwtKey, logger: logger}
}
...
func (s *AuthService) Register(c context.Context, username, password, email string) error {
	...
	err = s.userRepo.CreateUser(c, username, string(hashedPassword), email)
	if err != nil {
